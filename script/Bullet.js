function bullet(i)						//Konstruktor eines Feuerballs f�r Mario als Sch�tzen
{
	this.i = i;							//damit ist die Kugel eindeutig identifizierbar
	this.x = 0;							//world-Koordinaten -> f�r Animation in GameLoop()
	...
	$('#world').append('<div id=\'bullet' + i.toString() + '\'></div>');//erzeuge div f�r die Kugel und setze sie in die Spielwelt ein
	this.body = $('#bullet'+i.toString());								//hiermit l�sst sich das div ansprechen
	...
	this.body.css( { 'margin' : '0' ,
					 ...
					 'position' : 'absolute' ,					//Positionierung des Balls erfolgt relativ zum �bergeordneten world-Div
					 'left' : '0px' ,							//zun�chst mal irgendwo
					 'bottom' : '0px' ,
...
{	
	...
	j_alt = 14 - Math.floor( this.y / 32 );  	//vor move(): Ball auf H�he des j-ten Elements des levelarrays
	...
	j_neu = 14 - Math.floor( ( this.y + Delta_y ) / 32 ); //Ziel: Ball will auf H�he des j-ten Elements des levelarrays gelangen
	if( j_neu > 14 )			//Ball w�rde den unteren Rand des levelarrays �berschreiten, was nicht definiert ist!!!
		j_neu = 14;
	
	if( j_neu - j_alt == 1 )	//Ball f�llt in die n�chste Kachel hinein
	{
	...
	else if( j_neu - j_alt == 2 )	//Ball f�llt in die �bern�chste Kachel hinein -> die n�chste muss daher auch �berpr�ft werden!!!
	{
		...
		{	
			//Ball f�llt durch n�chste Kachel durch und landet auf �bern�chster:
			Delta_y = (15 - j_neu) * 32 - this.y;		//restliche Fallstrecke bis zum Boden
			this.velocity_y = 13;						//der Ball hoppst sofort weiter
		}
	}
	else if( j_neu - j_alt == 3 )	//Ball f�llt in die �ber�bern�chste Kachel hinein -> untersuche auch die beiden dar�berliegenden Kacheln!!!
	{
		...
		{	
			//Ball f�llt durch die beiden n�chsten Kacheln durch und landet auf �ber�bern�chster:
			Delta_y = (15 - j_neu) * 32 - this.y;		//restliche Fallstrecke bis zum Boden
			this.velocity_y = 13;						//der Ball hoppst sofort weiter
		}
	}
