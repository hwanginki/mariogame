function LoadLevel()
{
	...
	{
		mario = new Hero('mario','mario-img/mario-sprites.png');//Erzeuge eine neue Spielfigur
		//Events zuweisen (nicht fr�her, weil mario schon geladen sein muss!!!):
	...
	//s�mtliche M�nzen aus �lteren Spieldurchg�ngen l�schen, z.B. nachdem man gestorben ist und das Level neu geladen wurde: 
	for( var k = coins.length ; k-- ; )
		coins[k].body.destroy().remove();			//spritely beenden und M�nzen-Div vernichten
	coins = new Array();	
	
	//alle alten M�nzboxen l�schen:
	for( var k = coinboxes.length ; k-- ; )
	{
		coinboxes[k].body.remove();					//Block-Div entfernen
		coinboxes[k].coin.remove();					//M�nzen-Div vernichten, falls es noch existiert
	}
	coinboxes = new Array();	
	
	//alle alten Vielfach-M�nzboxen l�schen:
	for( var k = multiple_coinboxes.length ; k-- ; )
	{
		multiple_coinboxes[k].body.remove();		//Block-Div entfernen
		multiple_coinboxes[k].coin.remove();		//M�nzen-Div vernichten, falls es noch existiert
	}
	multiple_coinboxes = new Array();	
	
	//l�sche alle alten Sternboxen:
	...
	for( var k = plants.length ; k-- ; )
	{
		...
		if( plants[k].frame )
			plants[k].frame.remove();				//den Rahmen bei den R�hrenpflanzen entfernen
	}
	plants = new Array();
	
	//L�sche alle Gegner aus �lteren Spieldurchg�ngen:
	for( var k = enemies.length ; k-- ; )
	{
		enemies[k].body.destroy().remove();			//spritely beenden und Gegner-Div vernichten
		if( enemies[k].shell )
			enemies[k].shell.destroy().remove();	//Schildkr�tenpanzer vernichten
	}
	enemies = new Array();
	...
	context.fillRect(0,0,canvas.width,canvas.height);		//transparente Hintergrundfarbe f�r das Canvas
	
	objects = new Image();
		objects.onload = function() 
					{
						for( var i = 0 ; i < levelarray.length ; i++ )		//s�mtliche Levelspalten: von links nach rechts im Level
						for( var j = 0 ; j < 15 ; j++ )						//s�mtliche Levelzeilen: von oben nach unten je Spalte
						{	
							//zeichne das Level Kachel f�r Kachel:
							switch(levelarray[i][j])
							{
								...
								case 'brown_block':
									...
									if( i > 0 && i < levelwidth / 32 - 1 && j > 0 )	//f�r den Rand des Levels
									...
									context.drawImage(objects,514,194,32,32,i*32,j*32,32,32);	//zeichne braunen Block dr�ber
									break;
								case 'stone':
									...
									context.drawImage(objects,550,160,32,32,i*32,j*32,32,32);	//zeichne Stein dr�ber
									break;
								case 'pipe_top_left':
								...
								case 'pipe_right_soil':
									...
									break;
								case 'coin':
									coins.push( new coin(i,j) );										//Erstelle eine M�nze
									...
									break;
								case 'coinbox':
									coinboxes.push( new coinbox(i,j) );									//Erstelle eine M�nzbox
								...
								case 'multiple_coinbox':
									multiple_coinboxes.push( new multiple_coinbox(i,j) );				//Erstelle eine Vielfach-M�nzbox
								...
								case 'staticplant':
									...
									break;
								case 'pipeplant':
									plants.push( new pipeplant(i,j) );									//Erstelle eine R�hrenpflanze
									break;
								case 'spikedturtle':
									enemies.push( new spikedturtle(i,j) );								//Erstelle eine Stachelschildkr�te
									break;
								case 'ballmonster':
									enemies.push( new ballmonster(i,j) );								//Erstelle ein Kugelmonster
									break;
								case 'greenturtle':
									enemies.push( new greenturtle(i,j) );								//Erstelle eine gr�ne Schildkr�te
									break;
								case 'mario':																	
									mario.x = i*32;														//Setze Mario an den Anfang des Levels
									mario.y = (14-j)*32;
									mario.finished = false;												//und resete alle n�tigen Eigenschaften
									mario.dead = false;
						...
						//Canvasinhalt als levelimage abspeichern und in #world einf�gen:
						...
						//Richtungstasten noch nicht gedr�ckt:
						leftkey_down = false;
						rightkey_down = false;
						...
						GameLoop();			//Dauerschleife, die das Spiel am Laufen h�lt...
					};
		objects.src = 'mario-img/mario-objects.png';	//lade die Objekt-Sprites: ?, Pilze, Graslandschaften, etc.
}

function DefaultLevel(level)
{	
	levelarray = new Array();
	//Lege Level spaltenweise an, beginnend links oben. Jede Spalte hat 15 Eintr�ge (= H�he des Levels)!!!
	if(level==1)
	{
		$('#levelloading').show();
		$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_4.png\')' } );

		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'mario' , 'pipe_top_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'pipe_top_right' , 'pipe_right' , 'pipe_right' , 'pipe_right' , 'pipe_right' , 'pipe_right' , 'pipe_right' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , '' , 'coin' , 'coin' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , '' , '' , '' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , '' , '' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , 'coin' , 'coin' , '' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'coin' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , '' , '' , '' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , '' , '' , '' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , 'coin' , 'coin' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , 'coin' , 'coin' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'coin' , 'coin' , '' , '' , 'coin' , 'coin' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'coin' , 'grass_top' ) );
		...
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'pipe_top_left' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'pipe_top_right' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );	
	...
	else if(level==2)	//Remake vom 1. Level von Super Mario Land
	{	
		if(savegame)
		{
			$('#levelloading').show(); 
			$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_1.png\')' } );
			savegame = false;
		}
		else
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_1.png\')' } ); } , 8000 );
	...
	else if(level==2)		//Graslandschaften
	{
		...
		levelarray.push( new Array( '','','','','','','','','','','','','','grass_top','soil' ) );
		levelarray.push( new Array( '','','','','','','','','starbox','','','','','grass_top','soil' ) );
		levelarray.push( new Array( '','','','','','','','','coinbox','','','','','grass_top','soil' ) );
		levelarray.push( new Array( '','','','','','','','','mushroombox','','','','','grass_top','soil' ) );
	...
	else if(level==3)		//Graslandschaften
	{
		...
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'starbox' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' ) );
	...
	else if(level==4)		//Gebirgslevel
	{
		...
		{
			$('#levelloading').show(); 
			$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_8.png\')' } );
			savegame = false;
		}
		else
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_8.png\')' } ); } , 8000 );
	...
	else if(level==6)		//Dschungel-Einstiegslevel
	{
		...
		{
			$('#levelloading').show(); 
			$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_3.png\')' } );
			savegame = false;
		}
		else
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_3.png\')' } ); } , 8000 );
	...
	else if(level==7)		//Dschungel-Einstiegslevel
	{
		...
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'starbox' , '' , '' , '' , '' , 'grass_top_left' , 'grass_top_left_corner' , 'soil' , 'soil' ) );
	...
	else if(level==8)		//Dschungel
	{
		...
		{
			$('#levelloading').show(); 
			$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_5.png\')' } );
			savegame = false;
		}
		else
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_5.png\')' } ); } , 8000 );
		
		levelarray.push( new Array( '','','','','','','','','','','','bush_middle_left','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','','','','','bush_middle','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','','','','','bush_right','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','','','','','mario','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','starbox','','','','','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','','','','','','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','coinbox','','','','','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','brown_block','','','','','grass_top','soil','soil' ) );
	...
	else if(level==9)		//Dschungel
	{
		...
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'starbox' , '' , '' , '' , 'mario' , 'grass_top' , 'soil' , 'soil' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'coinbox' , '' , '' , '' , 'grass_top' , 'soil' , 'planted_soil_left' ) );
	...
	else if(level==10)		//Wolkenlevel
	{
		...
		{
			$('#levelloading').show(); 
			$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_6.png\')' } );
			savegame = false;
		}
		else
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_6.png\')' } ); } , 8000 );
		
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' ) );	
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'mario' , 'grass_top' , 'soil' , 'soil' ) );	
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_left' , 'soil' ) );	
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_middle' , 'soil' ) );	
		levelarray.push( new Array( '' , '' , '' , 'starbox' , '' , '' , '' , 'coinbox' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_right' , 'soil' ) );	
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'mushroombox' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' ) );	
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'coinbox' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' ) );	
		levelarray.push( new Array( '','','','','','','','','','','','bush_left','grass_top','soil','soil' ) );
		levelarray.push( new Array( '','','','','','','','','','','','bush_middle_left','grass_top','soil','soil' ) );
	...
	else if(level==11)		//Wolkenlevel
	{
		...
		levelarray.push( new Array( '','','','','','','','','','','','','','','' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top_left' , 'grass_left' , 'stone' , '' ) );	
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'starbox' , '' , '' , '' , '' , 'mario' , 'grass_top' , 'soil' , 'stone' , '' ) );	
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_left' , 'stone' , '' ) );	
	...
	{	
		...
		{
			$('#levelloading').show(); 
			$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_7.png\')' } );
			savegame = false;
		}
		else
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_7.png\')' } ); } , 8000 );

		levelarray.push( new Array( '','','','','','','','','','','','','','','' ) );
		levelarray.push( new Array( '','','','','','','','','','','','','','','' ) );			
		levelarray.push( new Array( '' , '' , '' , '' , '' , 'starbox' , '' , '' , '' , '' , 'mario' , 'grass_top_left' , 'grass_left' , 'stone' , '' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_left' , 'stone' , '' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_middle' , 'stone' , '' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_right' , 'stone' , '' ) );
		levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'stone' , '' ) );
	...
	{	
		...
		{
			$('#levelloading').show(); 
			$('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_2.png\')' } );
			savegame = false;
		}
		else
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_2.png\')' } ); } , 8000 );
	...
	{	
		...
		levelarray.push( new Array('' , '' , '' , '' , '' , '' , 'starbox' , '' , '' , '' , '' , '' , 'mario' , 'grass_top' , 'soil' ) );
		levelarray.push( new Array('' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' ) );
	...
	{	
		if(savegame)
		{
			...
			setTimeout( function() { $('#levelloading').show(); $('#background').css( { 'background-image' : 'url(\'backgrounds/mario-background_4.png\')' } ); } , 8000 );
		...
		if(savegame)
		{
			...
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' , 'soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' , 'soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'Mario' , 'grass_top' , 'soil' , 'soil' , 'soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'stone' , 'stone' , 'stone' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'coin' , 'coin' , 'spikedturtle' , 'stone' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'stone' , 'stone' , 'stone' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' , 'soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'mushroombox' , '' , '' , '' , 'grass_top' , 'soil' , 'soil' , 'soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_middle' , 'soil' , 'soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top' , 'planted_soil_left' , 'soil' , 'soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'grass_top_right' , 'grass_right' , 'grass_right' , 'grass_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'pipeplant' , 'pipe_top_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' ) );
			...
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , 'coin' , '' , 'coin' , '' , '' , 'pipe_top_left' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left' , 'pipe_left' , 'pipe_left' , 'pipe_left' ) );
			levelarray.push( new Array( '' , 'coin' , '' , 'coin' , '' , '' , 'pipe_top_right' , 'pipe_right_soil' , 'pipe_right_soil' , 'pipe_right_soil' , 'pipe_right_soil' , 'pipe_right' , 'pipe_right' , 'pipe_right' , 'pipe_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			...
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'coin' , '' , 'stone' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'coin' , '' , 'stone' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'mushroombox' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'ballmonster' , 'brown_block' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'brown_block' , '' , '' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'bush_left' , 'brown_block' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'bush_middle_right' , 'brown_block' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'bush_middle_left' , 'brown_block' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'bush_right' , 'brown_block' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , 'coin' , 'coin' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , 'pipeplant' , 'pipe_top_left' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , 'pipe_top_right' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , 'stone' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , 'stone' , 'stone' , 'stone' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , 'stone' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , 'stone' , '' , '' ) );
			...
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , 'spikedturtle' , 'stone' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			...
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , 'mushroombox' , '' , '' , 'stone' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , 'stone' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' , '' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'pipe_top_left' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' , 'pipe_left_soil' ) );
			levelarray.push( new Array( '' , '' , '' , '' , '' , '' , 'pipe_top_right' , 'pipe_right' , 'pipe_right' , 'pipe_right' , 'pipe_right' , 'pipe_right_soil' , 'pipe_right_soil' , 'pipe_right_soil' , 'pipe_right_soil' ) );
...
function Ending()									//Endsequenz (Mario trifft Daisy) und Credits
{
	...
	if(!custom)	eraseCookies();						//Cookies l�schen
	
	//Alle unn�tig gewordenen divs l�schen:
	//s�mtliche M�nzen aus �lteren Spieldurchg�ngen l�schen, z.B. nachdem man gestorben ist und das Level neu geladen wurde: 
	for( var k = coins.length ; k-- ; )
		coins[k].body.destroy().remove();			//spritely beenden und M�nzen-Div vernichten
	coins = new Array();	
	...
	//Hintergrund f�r die Endsequenz:
	...
	objects = new Image();
		objects.onload = function() 
					{
						for( var i = 0 ; i < endarray.length ; i++ )		//s�mtliche Levelspalten: von links nach rechts im Level
						for( var j = 0 ; j < 15 ; j++ )						//s�mtliche Levelzeilen: von oben nach unten je Spalte
						{	
							//zeichne das Level Kachel f�r Kachel:
							switch(endarray[i][j])
	...
	$('#mario').css( { 'margin' : '0' ,
				       ...
				       'width' : '80px' , 
				       'height' : '80px' , 
				       'background-image' : 'url(\'mario-img/mario-sprites.png\')' , 
				       'background-position' : '0px -243px' , 		//nach rechts schauender, gro�er Mario
				       'position' : 'absolute' ,					//Positionierung der Spielfigur erfolgt relativ zum �bergeordneten world-Div
				       'left' : '-280px' ,								
				       'bottom' : '96px' ,							
				       'z-index' : '100'  } );						//Peach ist vor Mario
	$('#mario').sprite( { fps: 9 , no_of_frames: 2, rewind: true } );//damit Mario l�uft
	...
	$('#speech').css( { 'margin' : '0' ,
				        ...
				        'width' : '0px' , 
				        'height' : '37px' , 
				        'position' : 'absolute' ,					//Positionierung des Textes erfolgt relativ zum �bergeordneten world-Div
				        'left' : '450px' ,								
				        'bottom' : '230px' ,
						'text-align' : 'right' ,
	...
	$('#mario').animate( { 'left' : '30' } , 1000 , function() 																//Mario l�uft von links rein
	{
		...
		{ 
			$('#speech').animate( { 'width' : '66px' } , 400 , function() 													//Peach ruft Mario
			{
				setTimeout( function() 
				{
					...
					$('#peach').css( { 'background-position' : '-138px -80px' } ).sprite( { fps: 6 , no_of_frames: 4 } ).animate( {'left':'336px'} , 2000 , function() //Peach l�uft Mario entgegen
					{ 
						$('#peach').destroy().css( { 'background-position' : '0px -80px' } ); 
					} );
					setTimeout( function() 																					//Mario l�uft Peach entgegen
					{
...
{	
	...
	//Zur�ck ins Hauptmen� und alle unn�tig gewordenen divs l�schen:
	setTimeout( function() 
					{
						//s�mtliche M�nzen aus �lteren Spieldurchg�ngen l�schen, z.B. nachdem man gestorben ist und das Level neu geladen wurde: 
						for( var k = coins.length ; k-- ; )
