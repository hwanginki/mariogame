function createCookies(days) 
{
	...
	var cookie_ende = "; expires=" + date.toGMTString() + "; path=/";	//Anh�ngsl an das Name-Wert-Paar des Cookies
	
	//Erstelle Cookies:
	document.cookie = "lives=" + mario.lives.toString() + cookie_ende;	//wieviele Leben hatte der Spieler?
	document.cookie = "coins=" + mario.coins.toString() + cookie_ende;	//wieviele M�nzen hatte der Spieler?
	document.cookie = "level=" + mario.level.toString() + cookie_ende;	//welches Level hatte der Spieler erreicht?
	document.cookie = "state=" + mario.state + cookie_ende;				//war Mario gro� oder klein?
	document.cookie = "shooter=" + mario.shooter.toString() + cookie_ende;//konnte man schie�en?
}
...
{	
	...
	for( var i = cookieArray.length ; i-- ; )
	{
		...
		while( c.charAt(0) == ' ' ) 
			c = c.substring(1,c.length);								//unn�tige Leerzeichen entfernen
...
{
	createCookies(-1);			//die Cookies sind gestern abgelaufen und werden deswegen gel�scht
}