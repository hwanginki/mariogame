	...
	switch(event.keyCode)		//welche taste?
	{
		...
		case 89://y hoch
			...
			if( !Strg_down )
				mario.fast = false;	//Mario läuft wieder mit normaler Geschwindigkeit
			break;
		case 40://Pfeiltaste nach unten hoch 아래버튼
			...
			break;
		case 39://Rechte Pfeiltaste hoch 오른쪽버튼
			rightkey_down = false;
			if( !mario.crouching )				//beim Ducken sollen Richtungsänderungen ignoriert werden
			{
			...
			break;
		case 37://Linke Pfeiltaste hoch 왼쪽버튼
			leftkey_down = false;
			if( !mario.crouching )				//beim Ducken sollen Richtungsänderungen ignoriert werden
			{
			...
			break;
		case 38://Pfeiltaste nach oben hoch 위버튼
			mario.jumping = false;
			break;
		case 83://s hoch
			s_down = false;
...
function controllsHandler() // 버튼 눌렀을 때 나타내는 창 
{
	$('#controllpage').slideToggle(); // 버튼 눌렀을 때 나타내는 창
}

function exit_controllpageHandler() // 창에서 나갈 때
{
	$('#controllpage').fadeToggle(); // X버튼 눌렀을땐 사라지는 창
}

function itemnotesHandler() // 버튼 눌렀을 때 나타내는 창
{
	$('#back_to_menu').hide();
	$('#itemnotepage').slideToggle();
}

function exit_itemnotepageHandler() // 버튼 눌렀을 때 사라지는 창
{
	$('#back_to_menu').slideToggle();
	$('#itemnotepage').fadeToggle();
}
